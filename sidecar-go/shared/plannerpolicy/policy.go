...
var policyBodyLines = []string{
	...
	"Use completed with action_type=none for successful informational answers where Milo explains, drafts, summarizes, teaches, or plans without claiming a real-world side effect.",
	"Explanation-only, draft-only, and plan-only answers are successful non-action answers when no real device, app, file, report, provider, permission, or external message action occurred.",
	"Mark completed for a real-world action only when the supplied prompt and runtime context already verify the outcome.",
	...
	"Use blocked only when a requested real operation cannot proceed because it is unsafe, impossible, missing permission or capability, missing required proof, blocked by required confirmation, or stopped by provider/runtime failure.",
	"Explaining why a real operation cannot be performed may be a completed informational answer if Milo does not claim the operation happened.",
...
func ValidateResponse(resp contracts.RelayTurnResponse) error {
	...
	if completion == "completed" {
		...
		if resp.RequiresUserChoice {
			return errors.New("completed_cannot_require_user_choice")
		}
	}
