...
- during launch, show redeem-code only
- later public mode can re-enable:
  - start free trial
  - redeem code
  - subscribe

If the verified email already used the one-time trial in later public mode:
...
- access-code only launch in the early rollout
- codes are single-use only
- burn permanently after redemption
- redemption available during setup and on the paywall/settings path
- each redeemed code stores the verified email and redeemed date
- launch-stage redeemed code grants 30 days of access by default
- later public mode still keeps access codes working normally
- reinstall/new-device loses device-only access in older v1 assumptions, but paid email-owned access is now the longer-term target
...
After payment or access-code redemption following interruption:
