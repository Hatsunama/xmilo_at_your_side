...
## First-pass authority lookup

- Active authority now resolves from `C:\xMilo\Source_Of_Truth_And_Phase_List_Master\` first.
- The dedicated startup/setup authority file is:
  - `C:\xMilo\Source_Of_Truth_And_Phase_List_Master\XMILO_STARTUP_AND_SETUP_FLOW__SOURCE_OF_TRUTH.txt`
- `C:\xMilo\xmilo_at_your_side\docs\authority\xMilo_v1\source_of_truth\` remains a mirror/helper pack only.
- Legacy file locations remain preserved as mirror/history paths until Main Hub explicitly retires them.
