...
This package is the single integrated authority pack for the current MiloClaw / PicoClaw rebuild.

## Purpose

This package exists to stop architectural drift.
It keeps the patched **Mind v4** pack, the **PicoClaw Go runtime checklist**, and the **Wizard Lair blocker/product locks** together in one place.

## Core runtime model

- **Mind v4** is the policy and durable knowledge layer.
- **PicoClaw Go** is the only runtime authority.
...
2. `PicoClaw_Go_Responsibility_Checklist.md` for engine/runtime implementation scope
...
- PicoClaw retries a failed relay call **3 times** with backoff:
  - 2s
  - 5s
  - 10s
- if all fail:
...
Source: `PicoClaw_Go_Responsibility_Checklist.md` → `## 17. WebSocket Reliability`
...
- `PicoClaw_Go_Responsibility_Checklist.md` = engine implementation checklist
