...
// Service manages the full entitlement lifecycle: trial, invite, subscription.
//
...
//   - invite:       code redeemed → grant with started_at = now, expires_at = now + days.
...
// RedeemInvite validates and consumes a single-use invite code, granting access.
...
// ListInviteCodes returns all invite codes with their usage state.
...
// generateCode returns a random 8-char alphanumeric invite code (no ambiguous chars).
