...
type Config struct {
	...
	WebsiteHandoffSecret        string
	WebsiteHandoffAudience      string
	WebsiteHandoffMaxAgeSeconds int
}
...
func Load() (Config, error) {
	cfg := Config{
		...
		InviteBetaDays: getenvInt("XMILO_INVITE_BETA_DAYS", 30),
		AccessMode:     getenv("XMILO_ACCESS_MODE", "code_only"),

		RevenueCatWebhookAuth:       getenv("REVENUECAT_WEBHOOK_AUTH", ""),
		WebsiteHandoffSecret:        getenv("RELAY_HANDOFF_SECRET", ""),
		WebsiteHandoffAudience:      getenv("RELAY_HANDOFF_AUDIENCE", "xmilo-sol-site"),
		WebsiteHandoffMaxAgeSeconds: getenvInt("RELAY_HANDOFF_MAX_AGE_SECONDS", 300),
	}
	...
	if cfg.WebsiteHandoffSecret == "" {
		return Config{}, errors.New("missing RELAY_HANDOFF_SECRET")
	}
	return cfg, nil
}

func (c Config) AccessCodeOnly() bool {
	return c.AccessMode == "code_only"
}

func (c Config) TrialAllowed() bool {
	return !c.AccessCodeOnly()
}

func (c Config) SubscriptionAllowed() bool {
	return !c.AccessCodeOnly()
}
