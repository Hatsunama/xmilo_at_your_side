...
// Redeems a single-use invite code and grants 5-day access.
func (a *App) handleAuthInvite(w http.ResponseWriter, r *http.Request) {
	...
	writeJSON(w, http.StatusOK, map[string]any{"ok": true, "message": "invite code accepted"})
}
...
func (a *App) handleTurn(w http.ResponseWriter, r *http.Request, claims map[string]any) {
	...
	resp, err := a.turns.Execute(r.Context(), req)
	if err != nil {
		writeJSON(w, http.StatusInternalServerError, map[string]any{"error": err.Error()})
		return
	}
